...
/** The real vault owner, read from the generated deployment config. */
const VAULT_OWNER = ADDRESSES.smartAccount;

export function DepositorView() {
  const { address, isConnected } = useAccount();
  const [isShielding] = useState(false);
  ...
  /**
   * Shielding is not wired to the browser, deliberately.
   *
   * The shield path is fully implemented and works — see
   * railgun-sidecar/src/shield.ts, and the on-chain proof at
   * sepolia.etherscan.io/tx/0x15a77bfa…cfa361. But the sidecar holds the wallet
   * mnemonic and runs on an internal-only Docker network with no published
   * port. Exposing it to a browser would undo that isolation for a demo.
   *
   * The previous implementation set a fabricated hash ('0xabc123...def456')
   * after a 2-second timeout, which is indistinguishable from a real deposit to
   * anyone looking at the screen. Reporting a deposit that did not happen is
   * worse than reporting none.
   */
  const handleShield = (e: React.FormEvent) => {
    e.preventDefault();
    setNotice(
      'Shielding runs through the operator sidecar, which is intentionally not reachable from a browser — it holds the wallet mnemonic. Run: docker compose exec -T enclave curl -X POST http://railgun-sidecar:8080/shield',
    );
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-light p-8 rounded-3xl anim flex flex-col gap-6 relative overflow-hidden group hover:border-white/20 transition-all duration-500 hover:shadow-[0_0_40px_rgba(99,102,241,0.05)]" style={{ '--d': '0.2s' } as React.CSSProperties}>
          ...
          <form onSubmit={handleShield} className="flex flex-col gap-4 mt-4 relative z-10">
            ...
            >
              <TextMorph>{txHash ? 'Funds Shielded' : 'How To Shield'}</TextMorph>
            </button>

            {notice && (
              <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-4 text-[12px] leading-relaxed text-amber-200/80">
                {notice}
              </div>
            )}
